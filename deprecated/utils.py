...
def ScanAudioFiles(root_dir, ver):
    '''
    ScanAudioFiles 函数用于扫描指定目录下的音频文件，并返回音频文件路径和对应的标签。
    该函数会遍历目录树，查找所有以 .wav 结尾的文件，并将它们的路径和标签存储在列表中。
    最终返回两个列表：音频文件路径列表和标签列表。
    '''
...
class SpeechCommand(Dataset):
    """
    SpeechCommand 是一个自定义的数据集类（继承自 PyTorch 的 Dataset 类），负责：
    1. 加载音频数据：从指定目录（train_dir、valid_dir、test_dir）读取音频文件
    2. 应用转换：对每个音频样本应用 transform（这里是 Padding() 操作）
    3. 管理数据访问：提供按索引获取音频数据和标签的接口
    
    从 utils.py 可以看出，SpeechCommand 类内部使用 ScanAudioFiles 函数扫描指定目录下的所有WAV文件，
    并将文件路径和对应的标签（如"yes"、"no"等）保存起来。当访问某个索引时，它会加载对应的音频文件并应用填充变换。
    （填充变换即将每个音频样本填充到指定长度：1s）
    """
    ...
    def __getitem__(self, idx):
        ...
        # sample, _ = torchaudio.load(audio_path) 返回两个值：
        # sample: 一个形状为 [channels, time] 的二维张量，其中：
            # channels: 音频的声道数（1表示单声道，2表示立体声等）
            # time: 音频采样点数量
        # _: 该音频的采样率（通常是16000Hz，在此代码中被忽略）
        # 不同于 librosa.load()，torchaudio 默认保留原始音频的所有声道，而不会自动将其混合为单声道。
        if self.transform:
...
class Preprocess:
    '''
    Preprocess 类是一个音频预处理器，它负责将原始音频数据转换成
    适合神经网络输入的特征表示，同时应用各种数据增强技术以提高模型的鲁棒性。
    '''
    def __init__(
        self,
        noise_loc, # 噪声文件存放目录
        ...
        specaug=False, # 是否使用SpecAugment数据增强(频域增强)
        sample_rate=SR, # 采样率
        frequency_masking_para=7, # 频域增强时，频率掩码的最大宽度
    ...
    def __call__(self, x, labels, augment=True, noise_prob=0.8, is_train=True):
        '''
        __call__方法是Preprocess类的核心方法，用于对输入音频数据进行预处理和增强。
        当实例化Preprocess类后，可以直接调用该方法来处理音频数据。
        
        参数说明：
        x: [batch_size, 1, sample_len]
        labels: [batch_size]，表示每个音频样本的标签
        augment: 是否进行数据增强
        noise_prob: 噪声增强的概率
        is_train: 是否在训练阶段
        '''
        ...
        if augment:
            for idx in range(x.shape[0]):
                ...
                # 将随机选择的背景噪声加载到设备上，并随机选择一个片段进行叠加
                ...
                if is_train:
                    # 在训练过程中随机偏移音频（最多±10%），模拟不同的说话速度和起始点
        ...
        # 使用LogMel类将音频波形转换为梅尔频谱图
        x = self.feature(x)
        if self.specaug:
            # 当specaug=True时应用频谱增强
            # 实现两种掩码操作：
            # 频率掩码：随机将特定频率区间置零
            # 时间掩码：随机将特定时间段置零
...
class Padding:
    """
    该类用于将音频样本填充到1秒的长度，以适应BC-Resnet模型的输入一致性要求。
    使用zero padding填充音频样本，使其长度为1秒（16000个采样点）。
    
    在GSCv2数据集论文《Speech Commands: A Dataset for Limited-Vocabulary Speech Recognition》中
    存在一条特殊限制：将所有话语的标准持续时间限制为一秒。
    故从GSCv2数据集中读取的关键词样本不可能超过1秒。每当读取到大于1秒的样本时，raise ValueError("no sample exceed 1sec in GSC.")
    """
...
def split_data(base, target, valid_list, test_list):
    ...
    for root, _, files in os.walk(base): # 目录递归遍历方式，用于遍历指定目录及其所有子目录。
        # root (dirpath): 当前正在访问的目录的完整路径
        # _ (dirnames): 当前目录下的所有子目录名称列表（此处使用下划线表示忽略这个值）
        # files (filenames): 当前目录下的所有文件名称列表
        for file_name in files:
            ...
            class_name = root.split("/")[-1] # 获取当前文件所在目录的最后一个部分作为类名
            for item in trg_base_dirs: # 在{loc}_split/train & valid & test目录下创建对应的类名目录，例如gscv1中存在30个类的音频，将会在每个文件夹中创建30个子目录
            ...
            shutil.copy(org_file_name, target_path) # 将音频文件复制到目标目录
            print(f"Copied {org_file_name} to {target_path}")


def SplitDataset(loc):
    target_loc = "%s_split" % loc # 临时分割目录"{loc}_split"
